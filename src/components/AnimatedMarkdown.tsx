...
const MarkdownAnimateText: React.FC<SmoothTextProps> = ({
  ...
  const generatePatterns = React.useMemo(() => {
    ...
    const fullPatterns = Object.keys(customComponents).map(
      (pattern) => new RegExp(pattern.slice(1, -1))
    );
    const partialPatterns: string[] = fullPatterns
  ...
  const processCustomComponents = React.useCallback(
    (text: string): React.ReactNode[] => {
      ...
      const regex = new RegExp(
        `(${fullPatterns.map((pattern) => pattern.source).join("|")})`,
        "g"
      );
      ...
      if (fullPatterns.length === 0) {
        return text.split(/\b\w+\b/g).filter(Boolean);
      }
      ...
      for (const match of remainingText.matchAll(regex)) {
        ...
        const matchPattern = fullPatterns.find((pattern) =>
          new RegExp(pattern).test(matchText)
        );
        if (
          matchPattern &&
          customComponents[matchPattern as unknown as string]
        ) {
          ...
          if (matchText.startsWith("<")) {
            ...
            const propsMatch = matchText.match(
              /\s+([\w-]+)=(?:"([^"]*)"|{([^}]*)})/g
            );
            ...
            parts.push(
              <TokenizedText
                input={
                  <CustomComponent key={match.index} content={matchText} />
                }
                sep={sep}
                animation={animation}
                animationDuration={animationDuration}
      ...
      if (lastIndex < remainingText.length) {
        ...
        if (partialMatch && partialMatch.index) {
          const beforePartial = remainingText.slice(
            lastIndex,
            lastIndex + partialMatch.index
          );
  ...
  const components: any = React.useMemo(
    () => ({
      ...
      code: ({ node, className, children, ...props }: any) => {
        ...
        return (
          <div {...props} style={animationStyle} className={`relative`}>
            <button
              onClick={handleCopy}
              style={{
